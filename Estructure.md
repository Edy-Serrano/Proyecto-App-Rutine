...
* **`dashboard_screen.dart`**: Pantalla de "Inicio". Muestra el saludo inicial (según la hora del día), el **Reto Diario Interactivo** (de confort) con niveles y botones para ganar rachas especiales, el progreso circular del día actual y una lista de las tareas pendientes para hoy. Las tareas canceladas se ordenan dinámicamente al final de la lista con diseño visual propio (línea roja y cruz). Soporta interacciones táctiles completas (deslizar para completar/borrar, toque corto para detalles e historial de tiempo, toque largo para editar y posponer).
