...
* **`task_provider.dart`**: El núcleo lógico de la app. Gestiona la lista en memoria de las tareas. Provee métodos para agregar, editar, eliminar, marcar tareas como completadas, **posponer tareas** y **cancelar tareas**. Calcula los historiales de tareas postergadas de forma dinámica (`getFullHistory`) mediante enlazado en cascada. Detecta automáticamente tareas vencidas (`_checkOverdueTasks`) cancelándolas con el motivo 'Vencida'. Filtra inteligentemente las tareas canceladas para que no afecten los promedios de éxito. Se encarga de:
...
* **`dashboard_screen.dart`**: Pantalla de "Inicio". Muestra el saludo inicial, un **ícono de notificaciones** (Campana con contador dinámico) en la esquina superior, el **Reto Diario Interactivo**, el progreso circular del día actual y la lista de tareas pendientes. Soporta interacciones táctiles completas (deslizar para completar/borrar, toque largo para editar).
* **`agenda_screen.dart`**: Vista de calendario. Usa un selector semanal deslizante. Incluye un botón superior que alterna entre la lista clásica y una innovadora **Vista de Línea de Tiempo (Estilo Google Calendar)** que mapea visualmente los bloques de tiempo. Permite ver las tareas asignadas a cualquier día y soporta acciones rápidas.
* **`stats_screen.dart`**: Panel de métricas dinámico. Recalcula al instante promedios semanales y mensuales. Incluye la creación de **Metas Mensuales Personalizadas**, permitiendo establecer objetivos de horas por categoría o por tareas específicas (ej. "Cepillarse"). Incluye exportación en CSV (ahora dividido en Tareas, Retos y Metas).
...
* **`hive_service.dart`**: Inicializa la base de datos local utilizando cifrado nativo **AES-256**. Provee la envoltura para leer/escribir las Tareas, Metas y Preferencias. Además de la exportación manual (`exportSecureBackup`), implementa un **Auto-Backup Silencioso** (`createAutoBackup`) que, usando `permission_handler`, guarda constantemente una copia viva de los datos en la carpeta pública `Documents/RutineBackup` del celular, haciéndola resistente a desinstalaciones de la app.
